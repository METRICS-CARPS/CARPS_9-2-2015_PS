...
# code for trial type
# 1 = no surprise, 2 = surprise trial, 3 = first control trial, 4 = second control trial, 5 = third control trial, 6 = fourth control trial
d.tidy <- d.tidy %>%
  mutate(trial_type = ifelse(trial_num < 156, 1, ifelse(trial_num == 156, 2, ifelse(trial_num == 157, 3, ifelse(trial_num == 158, 4, ifelse(trial_num == 159, 5, 6))))))
...
```{r}
reportedValues <- data.frame(c("NA","NA",.89,.30,.25,.80,.70,.75,.80,.75,.75,.85,.70,.80,.80,.80,.75,.7))
accuracies <- d.tidy %>%
...
color_accuracy <- bind_rows(counts[4,3:4], counts[7,3:4])
chisq.test(color_accuracy, correct=FALSE) # had to turn off Yates’ continuity correction.
abs(phi(color_accuracy))

# second chi square test - location accuracy between surprise trial and first control trial
# reported: χ2(1, N = 40) = 10.00, p < .005, ϕ = .50
location_accuracy <- bind_rows(counts[5,3:4], counts[8,3:4])
chisq.test(location_accuracy, correct=FALSE)
abs(phi(location_accuracy))
